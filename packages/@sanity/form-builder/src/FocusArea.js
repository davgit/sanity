...
export class FocusArea extends React.Component<Props, State> {
  ...
  handleDrop = (ev: SyntheticDragEvent<*>) => {
    ...
    if (this.props.onUpload) {
      ...
      const items = Array.from(ev.dataTransfer.items).map(item => item.getAsFile())
      if (items.length === 0) {
        return
      }
      ev.preventDefault()
  ...
  render() {
    const {children, className, onUpload, getUploadOptions, ...rest} = this.props
